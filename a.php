...
function bypass_shortlinks($url, $separator = 0) {
    ...
    if (preg_match("#(luckydice.net|kalimbanote.com|cryptoflare.cc|myhealths.icu|clk.st|urlsfly.me|wefly.me|shortsfly.me|linksfly.me|clicksfly.me)#is", $host)) {
        ...
        } elseif (preg_match("#(go.bitss.sbs|546512.xyz|go.shtfly.com|go.revcut.net.co|l2.revcut.net|121989.xyz|go.urlcut.pro|131989.xyz|go.faho.us|141989.xyz|go.eazyurl.xyz|link.eazyurl.xyz|go.cutlink.xyz|151989.xyz|120898.xyz|bitcosite.com|161989.xyz)#is", $host)) {
      ...
      if (preg_match("#(http)#is", $link)) {
        ...
        $url1 = $r["url1"][0];
            ...
            while(true) {
                ...
                if ($r["token_csrf"][1][0] == "no-recaptcha-noresponse") {
                    $data = http_build_query([
                        $r["token_csrf"][1][0] => "true",
                        $r["token_csrf5"][1][0] => "true"
                    ]);
                }
                $r = base_short($url1, 0, $data, $url1, 0, join('', $cookie));
                $cookie[] = $r["cookie"];
                $final = $r["url6"];
